...
function getUpstreamConfig() {
    const mode = (process.env.UPSTREAM_MCP_MODE ?? 'stdio');
    const reconnectMs = parseInt(process.env.UPSTREAM_MCP_RECONNECT_MS ?? '5000', 10);
    if (mode === 'sse') {
        const url = process.env.UPSTREAM_MCP_URL;
        if (!url)
            throw new Error('UPSTREAM_MCP_URL must be set when UPSTREAM_MCP_MODE=sse');
        return { mode, url, reconnectMs };
    }
    // stdio mode
    const command = process.env.UPSTREAM_MCP_COMMAND;
    if (!command) {
        throw new Error('UPSTREAM_MCP_COMMAND must be set when UPSTREAM_MCP_MODE=stdio.\n' +
...
export class UpstreamMcpClient {
     ...
     */
    async connect() {
        this.client = new Client({ name: 'accountability-ledger-upstream-client', version: '0.1.0' }, { capabilities: {} });
        const transport = this.createTransport();
        // Handle unexpected disconnects — attempt reconnect
        transport.onclose = () => {
            this.connected = false;
            this.tools = [];
            console.warn(`[AL Upstream] Upstream MCP disconnected. Reconnecting in ${this.config.reconnectMs}ms…`);
        ...
        console.log(`[AL Upstream] Connected (${this.config.mode}). ` +
    ...
    async callTool(name, args) {
        if (!this.client || !this.connected) {
            throw new Error(`[AL Upstream] Cannot call tool '${name}': upstream MCP server is not connected.`);
        }
