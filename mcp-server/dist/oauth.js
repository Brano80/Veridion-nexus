...
function getConfig() {
    const auth_mode = (process.env.AL_AUTH_MODE ?? 'jwks');
 ...
 */
export async function validateToken(rawToken, traceparent) {
...
function parseTraceparent(traceparent) {
    ...
    if (!rawTrace || rawTrace.length !== 32)
        return {};
    if (!rawSpan || rawSpan.length !== 16)
        return {};
    const trace_id = [
        rawTrace.slice(0, 8),
        rawTrace.slice(8, 12),
